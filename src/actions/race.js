            ...
            openModal(h('div', null,
                ...
                h('div', { className: 'modal-actions' },
                    ...
                    mkBtn(isFig8 ? '🔀 Submit Chaos' : isSpecial ? 'Submit Result' : 'Submit Result', isFig8 ? 'btn btn-chaos' : 'btn btn-success', () => {
                        ...
                        // Re-sort: finishers first in order, DNFs at the bottom in their original order
                        const _finishers = finishOrder.filter(e => !e.dnf && !e.dq);
                        const _dnfs = finishOrder.filter(e => e.dnf || e.dq);
                        const _reordered = [..._finishers, ..._dnfs];
                        _reordered.forEach((e, i) => { e.pos = i + 1; });
                        finishOrder.length = 0;
                        _reordered.forEach(e => finishOrder.push(e));
                        ...
                        if (!isFig8 && !isSpecial) {
                            ...
                            const _summary = _raceSnap && _raceSnap.result && _raceSnap.result.summary;
                            if (_summary) {
                                showSummaryToast(_summary, (getSeries(seriesId) && getSeries(seriesId).color) || '#F59E0B', (getSeries(seriesId) && getSeries(seriesId).short) || '');
                            }
                            setTimeout(function () {
